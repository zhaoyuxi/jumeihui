...
public class VideoAdapter extends RecyclerView.Adapter {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        VideoBean bean = lists.get(position);
        if (bean.getImage_url().isEmpty()) {
            vh.videoImg1.setImageBitmap(bean.getThumbnail());
        } else {
            new ImgLoader(context).disPlayimg(bean.getImage_url(),vh.videoImg1);
        }
    }
